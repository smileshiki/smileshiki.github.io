...
$$
\begin{aligned}
\int_{t=0}^T{\text{d}x_t}&=\lim_{\Delta \rightarrow 0} \left[ \left( z_{\Delta}-z_0 \right) +\left( z_{2\Delta}-z_{\Delta} \right) +\cdots +\left( z_T-z_{T-\Delta} \right) \right] 
\\
...
即$S_T=S_0\text{e}^{\left( \mu -\frac{1}{2}\sigma ^2 \right) T+\sigma \int_{t=0}^T{\text{d}z_t}}$。根据对数正态分布的性质，容易得到$\text{E}(S_T)=S_0\text{e}^{\mu T}$



&emsp;&emsp;由于市场钟不存在套利机会，一定存在