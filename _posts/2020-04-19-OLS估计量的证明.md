...
$$\begin{aligned} \min {G} \max {D} & \beta \underbrace{\mathbb{E}{P{G}(\mathbf{x})}\left[K L\left(\mathcal{U}(y) | P_{\theta}(y | \mathbf{x})\right)\right]}{(a)} \ &=+\underbrace{\mathbb{E}{P_{\text { in }}(\mathbf{x})}[\log D(\mathbf{x})]+\mathbb{E}{P{G}(\mathbf{x})}[\log (1-D(\mathbf{x}))]}_{(b)} \end{aligned} \qquad (2)$$
