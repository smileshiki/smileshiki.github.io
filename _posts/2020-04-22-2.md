...
其中$\mu \text{d}t$可以看作为确定性趋势，叫做漂移项，$\sigma \text{d}z_t$可以看作为随机扰动，可以得到
...
认为$\text{d}x_t$为每一时刻粒子位置的变化，$\mu \text{d}t$表示每时刻粒子恒定地增加$\mu$，而$\sigma$为例子位置变化的标准差。
...
- $\Delta = \frac{\partial c}{\partial S}$，衡量股价的影响
- $\Gamma = \frac{\partial^2 c }{\partial S^2}$，衡量$\Delta$的影响
- $\Theta = \frac{\partial c}{\partial T}$，衡量到期时间长短的影响
- $\Vega = \frac{\partial c}{\partial \sigma}$，衡量波动率的影响
- $\rho = \frac{\partial c}{\partial r}$，衡量无风险利率的影响