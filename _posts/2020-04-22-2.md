...
可以认为$\text{d}x_t$为每一时刻粒子位置的变化，$\mu \text{d}t$表示每时刻粒子恒定地增加$\mu$，而$\sigma$为例子位置变化的标准差。
...
在进行二阶泰勒展开时，仅保留$\text{d}z_t$和$\text{d}t$项，并且近似认为$\text{d}t=(\text{d}z_t)^2$（尽管并不严谨，因为$\text{E}$）。
